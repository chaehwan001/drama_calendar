# -*- coding: utf-8 -*-
"""
카테고리 '대한민국의 액션 드라마'에서 모든 컬럼(ㄱ/ㄴ/ㄷ …) + 페이지네이션까지 훑어서
각 항목 상세 페이지의 '제목 / 장르 / 방송사'만 추출하여 CSV 저장.
저장 전에 genre_name 자동 보정:
  1) 비었거나 결측 → "액션"
  2) "액션" 미포함 → "액션, {기존장르}"

출력: action_dramas_all.csv (UTF-8 with BOM), 컬럼: title, genre_name, channel_name
"""
...
CATEGORY_URL = "https://ko.wikipedia.org/wiki/%EB%B6%84%EB%A5%98:%EB%8C%80%ED%95%9C%EB%AF%BC%EA%B5%AD%EC%9D%98_%EC%95%A1%EC%85%98_%EB%93%9C%EB%9D%BC%EB%A7%88"
...
GENRE_KEYWORD = "액션"  # 이 스크립트의 장르 키워드
...
def main():
    ...
    # 🔹 장르 자동 보정 (빈값 → "액션", 미포함 시 맨 앞에 "액션, " 추가)
    df["genre_name"] = df["genre_name"].apply(fix_genre_value)

    out = "action_dramas_all.csv"
