# -*- coding: utf-8 -*-
"""
카테고리 '대한민국의 로맨틱 코미디 드라마'에서 모든 컬럼(ㄱ/ㄴ/ㄷ …) + 페이지네이션까지 훑어서
각 항목 상세 페이지의 '제목 / 장르 / 방송사'만 추출하여 CSV 저장.
저장 전에 genre_name 자동 보정:
  1) 비었거나 결측 → "로맨틱 코미디"
  2) "로맨틱 코미디" 미포함 → "로맨틱 코미디, {기존장르}"

출력: romantic_comedy_all.csv (UTF-8 with BOM), 컬럼: title, genre_name, channel_name
"""
...
CATEGORY_URL = "https://ko.wikipedia.org/wiki/%EB%B6%84%EB%A5%98:%EB%8C%80%ED%95%9C%EB%AF%BC%EA%B5%AD%EC%9D%98_%EB%A1%9C%EB%A7%A8%ED%8B%B1_%EC%BD%94%EB%AF%B8%EB%94%94_%EB%93%9C%EB%9D%BC%EB%A7%88"
