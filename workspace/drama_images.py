# -*- coding: utf-8 -*-
"""
Namuwiki drama image fetcher (requests-only, user-selectors + flexible fallback)

규칙:
  1) /w/{제목} 정확 페이지 먼저 열기
     - 드라마 판정은 1차: 네가 준 '방송시간/방송기간' CSS 셀렉터
                    2차: article 인포박스 테이블에서 strong/th/td 텍스트가
                        '방송 시간' / '방송 기간' / '제작사' 중 하나라도 있으면 드라마로 판정
     - 판정되면 og:image 저장
  2) 판정 실패하면 '{제목}(드라마)' 변형을 시도(공백 포함/미포함 모두)
  3) 그래도 실패면 검색 보조 (/Search?q=...)로 정확 일치 우선
"""
...
def has_user_cue_or_fallback(html: str) -> tuple[bool, str]:
    """
    True/False, how("user_sel"/"fallback"/"none") 반환
    1) 네가 준 CSS 셀렉터로 초고속 체크
    2) 실패 시: article 내 인포박스에서 '방송 시간/방송 기간/제작사' 라벨 매칭
    """
    soup = BeautifulSoup(html, "html.parser")
    ...
    # 2) fallback: article 내 앞쪽 테이블 확인
